...
export default function LogIn() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Force router refresh to sync Supabase auth cookies before navigating
      router.refresh();
      router.push('/dashboard');
      
    } catch (error: any) {
      // Catch any unexpected hard errors (like network failures or missing env variables)
          ...
          <form className="space-y-4" onSubmit={handleLogin}>
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5">
                ...
                <Link href="/forgot-password" className="text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors">
                  Forgot password?
                </Link>
              </div>
              <div className="relative">
